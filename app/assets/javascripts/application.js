// This is a manifest file that'll be compiled into application.js, which will include all the files
// listed below.
//
// Any JavaScript/Coffee file within this directory, lib/assets/javascripts, vendor/assets/javascripts,
// or any plugin's vendor/assets/javascripts directory can be referenced here using a relative path.
//
// It's not advisable to add code directly here, but if you do, it'll appear at the bottom of the
// compiled file.
//
// Read Sprockets README (https://github.com/rails/sprockets#sprockets-directives) for details
// about supported directives.
//
//= require js/modernizr-2.6.2.min
//= require jquery
//= require jquery_ujs
//= require turbolinks
//= require cocoon
//= require js/jquery-migrate.min
//= require js/jquery.validate.min
//= require js/jquery-easing-1.3
//= require js/jquery.sticky
//= require js/jquery.nicescroll.min
//= require js/jquery.inview
//= require js/validation
//= require js/jquery.tipTip.minified
//= require js/jquery.bxslider.min
//= require js/jquery.prettyPhoto
//= require js/twitter/jquery.tweet.min
//= require js/jquery.parallax-1.1.3
//= require js/shortcodes
//= require js/custom
//= require js/jquery-transit-modified
//= require js/layerslider.kreaturamedia.jquery
//= require js/greensock
//= require js/layerslider.transitions
//= require_tree .
